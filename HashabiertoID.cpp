...
private:
    ...
    int cant = 0;
    list <pair<double,string>> tabla[hashN]; //Armamos una tabla con: Lista 1 al indice 0, Lista 2 al indice 1... y así sucesivamente
    
public:
    bool isEmpty() const; //Para hacer la consulta si lista está vacía
    int funcHash(double clave); //Función hash con la que se almacenarán los datos
    void insertValor(double clave,string valor); //función insertar datos
    void removeValor(int clave); //Si queremos eliminar
    int buscarValor(double clave); //Si queremos buscar
    void printTabla();
    int getCant();
    int contarList();
};
...
int TablaHash::funcHash(double clave){
    return ((long int)round(abs(clave))) % hashN; //(unsigned int) clave%hashN
}


void TablaHash::insertValor(double clave,string valor){
    int valorHash = funcHash(clave); //Definiendo la entrada a la tabla hash con la clave
...
int TablaHash::buscarValor(double clave){
...
int main(){
    TablaHash TH;
    misDatos followers;
    vector <pair<double,string>> vectordatos;
    int contador =0;
    string dato;
    int n = 20000;
    ...
    for(int i=0; i<cantLineasLeer; i++){
        ...
        TH.insertValor(id,name);
        vectordatos.push_back(make_pair(id,name));
    }
    ...
    //Buscando datos que si están
    
    auto startSearch = std::chrono::system_clock::now();
    {
    for(int i=0;i<n;i++){
        double busca = vectordatos[rand()%vectordatos.size()].first;
    ...
    {
    for(int i=0;i<n;i++){
        double busca = random[rand()%random.size()];
        TH.buscarValor(busca);
        /*if(TH.buscarValor(busca) == -1){
            cout << "No : " << busca << endl;
        }
